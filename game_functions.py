...
def check_keydown_events(event, ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    elif event.key == pygame.K_F11:   # 进入和退出全屏
        ai_settings.fullscreen = not ai_settings.fullscreen
        if ai_settings.fullscreen:
            screen = pygame.display.set_mode(flags=pygame.FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.RESIZABLE)
        else:
...
def check_events(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
        elif event.type == pygame.KEYDOWN:  # 每次按键按下都被注册为一个KEYDOWN 事件
            check_keydown_events(event, ai_settings, screen, stats, sb, ship, aliens, bullets)
        elif event.type == pygame.KEYUP:    # 按键松开后
...
def check_bullet_alien_collosions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    # 检查是否有子弹击中了外星人
    # 如果是这样，就删除相应的子弹和外星人
    collisions = pygame.sprite.groupcollide(bullets, aliens, True, True)  #第一个True表示子弹消失，第二个True表示外星人消失
